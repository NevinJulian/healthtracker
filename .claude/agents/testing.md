...
5. If BOTH checks pass: merge the PR into `main` via the GitHub MCP using a **regular merge commit** (merge method `merge` — **never squash**), so the branch's individual commits are preserved on `main`. Then confirm the merge succeeded and the linked issue closed.
